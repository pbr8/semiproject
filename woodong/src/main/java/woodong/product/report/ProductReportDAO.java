...
public class ProductReportDAO {
	            ...
	            if(selectVal.equals("product_idx")) {
	               ...
	               ps.setString(1, searchVal);
		               ...
		               sql = "select * from ( "
		 	                  ...
		 	                  + "left join sp_product sp on pr.product_idx = sp.product_idx  "
		 	                  + "left join sp_user su on su.user_idx = pr.user_idx "
		 	                  + "where user_name like '%"+searchVal+"%') a) b  "
		 	                  + "where rnum >= ? and rnum <= ? ";
		 	               ps = conn.prepareStatement(sql);
		 		           ps.setInt(1, start);
		 		           ps.setInt(2, end);
	...
	public int getTotalCnt(String selectVal, String searchVal) {
		try {
			...
			String sql = "";
			if(searchVal==null) {
				sql = "select count(*) from sp_p_report";
				ps = conn.prepareStatement(sql);
			}else {
				if(selectVal.equals("product_idx")) {
		               sql = "select count(*) "
		               ...
		               sql = "select count(*) "
		                     + "from sp_user su, sp_p_report pr, sp_product sp "
		                     + "where su.user_idx = pr.user_idx and pr.product_idx = sp.product_idx and user_name like '%"+searchVal+"%'";
		               ps = conn.prepareStatement(sql);
		            }
			}
