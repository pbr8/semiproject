...
public class BbsReportDAO {
			...
			if((selectVal==null)) {
				...
				if(selectVal.equals("bbs_idx")) {
					sql = "select * "
							...
							+ "where su.user_idx = br.user_idx and su.user_idx = bs.user_idx and bs.bbs_idx = ?";
				}else {
					sql = "select * "
							+ "from sp_user su, sp_bbs_report br, sp_bbs bs "
							+ "where su.user_idx = br.user_idx and su.user_idx = bs.user_idx and su.user_name = ?";
				}
	...
	public int getTotalCnt() {
		try {
			conn = woodong.db.WoodongDB.getConn();
			String sql = "select count(*) from sp_bbs_report";
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
